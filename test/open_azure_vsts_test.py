...
class OpenAzureVSTSTest(unittest.TestCase):
    ...
    def test_unique_bugs_std_id(self):
        ...
        for bug_id, test_ids in bug_map_dict.items():
            ...
            work_items_search.fill_bug_id_input(bug_id)
            ...
            ok = std_id_empty or (str(field_val).strip() in [str(tid) for tid in test_ids])
            self.assertTrue(ok, f"Bug {bug_id}: STD ID field invalid! Value: '{field_val}' | Expected: empty or one of {test_ids}")

            time.sleep(1)  # Just for demo—remove or adjust for speed
            work_item.navigate_back()
